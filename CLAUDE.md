...
Сервис для обработки P2P-переводов между банком ВТБ и банком Кибит.
Протокол P2P Selector разработан ВТБ. ВТБ выступает клиентом — стучится в наш сервис.
Кибит — расчётный банк, фиксирует операции и зачисляет средства клиентам через Oracle АБС.

## Контекст проекта

### Бизнес-схема
```
ВТБ  ──→  p2p-selector  ←──→  Кибит (Oracle АБС)
```

ВТБ стучится в наш сервис по протоколу P2P Selector.
